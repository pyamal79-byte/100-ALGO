      ...
      <aside className={`fixed lg:static inset-y-0 left-0 z-50 w-64 bg-gray-900 border-r border-gray-800 flex flex-col h-full shrink-0 transform transition-transform duration-300 ease-in-out ${isOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
        ...
        <div className="p-6 flex-1 overflow-y-auto">
          ...
          <div className="flex p-1 bg-gray-950 rounded-xl border border-gray-800 mb-8">
            <button 
              onClick={() => { setSport('FOOTBALL'); setLeague('Toutes'); setIsOpen(false); }}
              className={`flex-1 py-2 text-xs font-bold rounded-lg transition-all ${sport === 'FOOTBALL' ? 'bg-gray-800 text-white shadow' : 'text-gray-500 hover:text-gray-300'}`}
            >
              FOOTBALL
            </button>
            <button 
              onClick={() => { setSport('BASKETBALL'); setLeague('Toutes'); setIsOpen(false); }}
              className={`flex-1 py-2 text-xs font-bold rounded-lg transition-all ${sport === 'BASKETBALL' ? 'bg-gray-800 text-white shadow' : 'text-gray-500 hover:text-gray-300'}`}
            >
              BASKETBALL
            </button>
          </div>
          ...
          <div className="space-y-1 mb-8">
            ...
            <button
              onClick={() => { setViewMode('DASHBOARD'); setIsOpen(false); }}
              className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-colors ${
                viewMode === 'DASHBOARD' ? 'bg-emerald-500/10 text-emerald-400' : 'text-gray-400 hover:bg-gray-800 hover:text-gray-200'
              }`}
            >
            ...
            <button
              onClick={() => { setViewMode('STANDINGS'); setIsOpen(false); }}
              className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-colors ${
                viewMode === 'STANDINGS' ? 'bg-emerald-500/10 text-emerald-400' : 'text-gray-400 hover:bg-gray-800 hover:text-gray-200'
              }`}
            >
            ...
            <button
              onClick={() => { setViewMode('ANALYSIS'); setIsOpen(false); }}
              className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-colors ${
                viewMode === 'ANALYSIS' ? 'bg-emerald-500/10 text-emerald-400' : 'text-gray-400 hover:bg-gray-800 hover:text-gray-200'
              }`}
            >
            ...
            <button
              onClick={() => { setViewMode('MATCHES'); setIsOpen(false); }}
              className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-colors ${
                viewMode === 'MATCHES' ? 'bg-emerald-500/10 text-emerald-400' : 'text-gray-400 hover:bg-gray-800 hover:text-gray-200'
              }`}
            >
          ...
          <div className="space-y-1">
            ...
            {currentLeagues.map(l => (
              <button
                key={l}
                onClick={() => { setLeague(l); setIsOpen(false); }}
                className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-colors ${
                  league === l ? 'bg-emerald-500/10 text-emerald-400' : 'text-gray-400 hover:bg-gray-800 hover:text-gray-200'
                }`}
              >
...
// --- Composant: Match Modal (Fiche Détaillée) ---

const MatchModal = ({ match, onClose }: { match: any | null, onClose: () => void }) => {
  if (!match) return null;

  // Determine if it's a mock match (has algoProbability) or real API match
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <div className="bg-gray-900 border border-gray-800 rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-800 bg-gray-900/50">
          <div className="flex items-center gap-3">
            <span className="px-2.5 py-1 rounded-md bg-gray-800 text-xs font-medium text-gray-300 border border-gray-700">
              {isMock ? match.league : match.competition?.name}
            </span>
        ...
        <div className="p-6 space-y-8">
          ...
          <div className="flex items-center justify-between">
            <div className="flex flex-col items-center gap-3 w-1/3">
              {match.homeTeam.crest ? (
                ...
                <div className={`w-16 h-16 rounded-full flex items-center justify-center text-xl font-bold ${match.homeTeam.color || 'bg-gray-800'}`}>
            ...
            <div className="flex flex-col items-center gap-2 w-1/3">
              {isMock ? (
                <>
                  ...
                  <span className="px-4 py-1.5 rounded-full bg-gray-800 text-sm font-medium text-white border border-gray-700">
                    {match.prediction}
                  </span>
                ...
                <div className="text-center">
                  ...
                  <span className="px-4 py-1.5 rounded-full bg-gray-800 text-sm font-medium text-emerald-400 border border-gray-700">
                    À venir
                  </span>
                </div>
              )}
            ...
            <div className="flex flex-col items-center gap-3 w-1/3">
              {match.awayTeam.crest ? (
                ...
                <div className={`w-16 h-16 rounded-full flex items-center justify-center text-xl font-bold ${match.awayTeam.color || 'bg-gray-800'}`}>
          ...
          {/* Stats & Value (Only for mock data currently, real API needs more endpoints for odds/stats) */}
          {isMock ? (
            <div className="grid grid-cols-2 gap-4">
                ...
                <div className="space-y-3">
                  ...
                  <div className="pt-3 mt-3 border-t border-gray-800">
                    ...
                    <p className="text-xs text-red-400">
                      {[...match.homeTeam.injuries, ...match.awayTeam.injuries].join(', ') || 'Aucune blessure majeure'}
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-gray-950 rounded-xl p-5 border border-gray-800 flex flex-col justify-between">
                <div>
                  <h4 className="text-sm font-medium text-gray-400 mb-4 flex items-center gap-2">
                    <Target className="w-4 h-4" /> Analyse de Valeur
                  </h4>
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <span className={`px-2 py-0.5 rounded text-sm font-mono font-bold border ${getValueBadge(match.valuePercentage)}`}>
                      {match.valuePercentage > 0 ? '+' : ''}{match.valuePercentage.toFixed(1)}%
                    </span>
                  </div>
                </div>
            ...
            <div className="bg-gray-950 rounded-xl p-5 border border-gray-800 text-center">
              <p className="text-gray-400 text-sm">
                Allez dans l'onglet "Matchs à venir" pour générer une analyse IA complète de cette rencontre.
              </p>
            </div>
          )}
            ...
            <table className="w-full text-left text-sm text-gray-400">
              <thead className="bg-gray-950/50 text-xs uppercase text-gray-500 border-b border-gray-800">
                <tr>
                  ...
                  <th className="px-4 py-4 font-medium text-center">N</th>
                  <th className="px-4 py-4 font-medium text-center">D</th>
                  <th className="px-4 py-4 font-medium text-center">BP</th>
                  <th className="px-4 py-4 font-medium text-center">BC</th>
                  ...
                  <tr key={row.team.id} className="hover:bg-gray-800/50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="font-medium text-gray-200">{row.team.name}</span>
                      </div>
                    </td>
                    <td className="px-4 py-4 text-center">{row.playedGames}</td>
                    ...
                    <td className="px-4 py-4 text-center">{row.goalDifference}</td>
  ...
  useEffect(() => {
    ...
    const fetchTeams = async () => {
      ...
      try {
        ...
        if (res.ok) {
          const data = await res.json();
          setTeams(data.map((row: any) => row.team));
        }
      } catch (err) {
        console.error(err);
      } finally {
  ...
  if (sport !== 'FOOTBALL') {
    return (
      <div className="flex flex-col items-center justify-center h-full text-gray-500 p-8 text-center">
        ...
        <p>L'analyse n'est disponible que pour le Football pour le moment.</p>
      </div>
    );
  }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              {teams.map(team => (
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors ${
                    selectedTeam === team.id ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' : 'text-gray-300 hover:bg-gray-800'
                  }`}
                >
        ...
        <div className="lg:col-span-2">
          {loadingAnalysis ? (
            <div className="flex flex-col items-center justify-center h-full text-gray-400 space-y-4">
              <div className="w-8 h-8 border-4 border-emerald-500 border-t-transparent rounded-full animate-spin"></div>
              <p>Génération de l'analyse IA en cours...</p>
            </div>
          ) : error ? (
            <div className="text-red-400 bg-red-500/10 p-4 rounded-lg border border-red-500/20">{error}</div>
          ) : analysis ? (
            <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6 space-y-6">
              <div className="flex items-center gap-4 border-b border-gray-800 pb-6">
                ...
                <div>
                  <h3 className="text-xl font-bold text-white">Rapport d'Analyse IA</h3>
                  <p className="text-sm text-gray-400">Basé sur les performances récentes et les statistiques avancées.</p>
                </div>
              </div>

              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className="bg-gray-950 p-4 rounded-xl border border-gray-800">
                  <p className="text-xs text-gray-500 mb-1">Matchs Joués</p>
                  <p className="text-xl font-bold text-white">{analysis.stats.matches_played}</p>
                </div>
                <div className="bg-gray-950 p-4 rounded-xl border border-gray-800">
                  <p className="text-xs text-gray-500 mb-1">Buts Marqués /m</p>
                  <p className="text-xl font-bold text-emerald-400">{analysis.stats.goals_for_avg}</p>
                </div>
                <div className="bg-gray-950 p-4 rounded-xl border border-gray-800">
                  <p className="text-xs text-gray-500 mb-1">Buts Encaissés /m</p>
                  <p className="text-xl font-bold text-red-400">{analysis.stats.goals_against_avg}</p>
                </div>
                <div className="bg-gray-950 p-4 rounded-xl border border-gray-800">
                  <p className="text-xs text-gray-500 mb-1">Position</p>
                  <p className="text-xl font-bold text-white">{analysis.stats.position}</p>
                </div>
              </div>
            ...
            <div className="flex flex-col items-center justify-center h-full text-gray-500 bg-gray-900/50 border border-gray-800 border-dashed rounded-2xl p-8 text-center">
              <Search className="w-12 h-12 mb-4 opacity-50" />
              <p>Sélectionnez une équipe dans la liste pour générer une analyse détaillée par l'Intelligence Artificielle.</p>
            </div>
          )}
        </div>
      </div>
...
const DashboardView = ({ sport, league, onAnalyze }: { sport: Sport, league: string, onAnalyze: (match: any) => void }) => {
  ...
  const codes: Record<string, string> = {
    ...
    'Serie A': 'SA',
    'La Liga': 'PD',
    'Toutes': 'ALL' // We will handle 'Toutes' differently if needed, or just fetch a default
  };
  ...
  useEffect(() => {
    ...
    const fetchMatches = async () => {
      ...
      setError('');
      try {
        // If 'Toutes', we might want to fetch multiple or just CL as default for dashboard
        const code = league === 'Toutes' ? 'CL' : codes[league];
        if (!code) return;
  ...
  if (sport !== 'FOOTBALL') {
    return (
      <div className="flex flex-col items-center justify-center h-full text-gray-500 p-8 text-center">
        <Calendar className="w-12 h-12 mb-4 opacity-50" />
        <p>Le calendrier n'est disponible que pour le Football pour le moment.</p>
      </div>
    );
  }
          ...
          {nextDays.map((date, i) => {
            ...
            return (
              <button
                ...
                className={`flex flex-col items-center justify-center min-w-[70px] lg:min-w-[90px] py-3 rounded-xl border transition-all ${
                  isSelected 
              ...
              {matches.map(match => (
                <div key={match.id} className="bg-gray-950 border border-gray-800 rounded-xl p-4 hover:border-gray-700 transition-colors">
                  ...
                  <div className="space-y-3 mb-4">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        {match.homeTeam.crest && <img src={match.homeTeam.crest} alt="" className="w-6 h-6 object-contain" />}
                        <span className="font-medium text-gray-200">{match.homeTeam.shortName || match.homeTeam.name}</span>
                      </div>
                    </div>
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        {match.awayTeam.crest && <img src={match.awayTeam.crest} alt="" className="w-6 h-6 object-contain" />}
                        <span className="font-medium text-gray-200">{match.awayTeam.shortName || match.awayTeam.name}</span>
                      </div>
                    </div>
                  </div>
                  <button 
                    onClick={() => onAnalyze(match)}
                    className="w-full py-2 bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-400 text-sm font-medium rounded-lg transition-colors border border-emerald-500/20"
                  >
                    Analyser ce match
                  </button>
                </div>
...
export default function App() {
  ...
  // Filtrage des matchs pour le dashboard
